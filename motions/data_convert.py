...
def main():
    ...
    root_pos_interp = interp1d(t_orig, root_data_orig[:, 0:3], axis=0, kind="linear")(
        t_new
    )
    ...
    if (7 + joint_data.shape[1]) != nq:
        print(
            f"Warning: CSV columns={7 + joint_data.shape[1]}, but pinocchio nq={nq}, may need to check or adjust script parsing."
        )
    ...
    for j in range(B):
        ...
        # Smoothing
        body_angular_velocities[:, j, :] = gaussian_filter1d(
            angular_vels, sigma=1, axis=0
        )
