...
def plot_pelvis_data_2d(source_data, target_data, source_pelvis_idx, target_pelvis_idx):
    ...
    plots_to_create = [
        ...
        (
            ...
            "Pelvis Linear Velocity Comparison",
            "Velocity (m/s)",
        ),
        (
            "body_angular_velocities",
            "Pelvis Angular Velocity Comparison",
    ...
    for i, (key, title, y_title) in enumerate(plots_to_create, 1):
        for data, idx, name in [
            (source_data, source_pelvis_idx, "Source"),
            (target_data, target_pelvis_idx, "Target"),
        ]:
            if key not in data:
                continue
            ...
            for d in range(dims):
                fig.add_trace(
                    go.Scatter(
                        ...
                        line=dict(
                            dash="solid" if name == "Source" else "dash", width=1.5
                        ),
                        showlegend=(
                            d == 0
                        ),  # Show legend only for the first component to avoid clutter
                    ),
                    row=i,
                    col=1,
                )
...
def visualize_pelvis_trajectories(
    source_data, target_data, source_pelvis_idx, target_pelvis_idx
):
    """
...
def update_pelvis_data(source_file, target_file, dry_run=False):
    ...
    try:
        ...
        if dry_run:
            print("\n--- DRY RUN MODE ---")
            print(
                "Displaying pelvis data for the first 5 frames. No files will be changed."
            )
        ...
        if num_frames_source < num_frames_target:
            print(
                f"Warning: Source file has fewer frames ({num_frames_source}) than target file ({num_frames_target}). Target will be truncated."
            )
            ...
            if num_frames_source > num_frames_target:
                print(
                    f"Warning: Source file has more frames ({num_frames_source}) than target file ({num_frames_target}). Source data will be truncated."
                )

        # Copy the pelvis data from source to target
        print("Copying pelvis data...")
        target_data_dict["body_positions"][:, target_pelvis_idx] = source_data[
            "body_positions"
        ][:num_frames, source_pelvis_idx]
        target_data_dict["body_rotations"][:, target_pelvis_idx] = source_data[
            "body_rotations"
        ][:num_frames, source_pelvis_idx]
