...
def verify_motion_file(npz_file):
    ...
    for key in data:
        ...
        if key == "dof_names":
            ...
            print(
                f"  Body rotations sample (first 2 frames, first 3 parts, quaternions):"
            )
            print(
                arr[:2, :3, :]
            )  # Show first 2 frames, first 3 parts quaternion rotations
        elif key == "body_linear_velocities":
            print(f"  Body linear velocities sample (first 2 frames, first 3 parts):")
            print(
                arr[:2, :3, :]
            )  # Show first 2 frames, first 3 parts linear velocities
        elif key == "body_angular_velocities":
            print(f"  Body angular velocities sample (first 2 frames, first 3 parts):")
            print(
                arr[:2, :3, :]
            )  # Show first 2 frames, first 3 parts angular velocities
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Verify and display motion data file contents"
    )
